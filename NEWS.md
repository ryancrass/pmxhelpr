...
* `df_doseprop()` now returns a `doseprop_stats`-classed data.frame. The visible columns and row count are unchanged (one row per metric with `Intercept`, `Power`, `LCL`, `UCL`, `PowerCI`, etc.), so existing code that reads those columns continues to work. The object additionally carries the plotting context as attributes: `obs` (filtered observation rows), `metric_var`, `exp_var`, `dose_var`, `ci`, and `method`. Code that asserts `class(df_doseprop(...)) == "data.frame"` should use `inherits(x, "data.frame")` instead.
...
* `plot_build_doseprop`: Public renderer that builds a dose-proportionality ggplot from any `doseprop_stats` object. Most users still go through `plot_doseprop()`; call `plot_build_doseprop()` directly when working from a manually-constructed or cached `doseprop_stats` object. The required column-name strings, observation rows, and `ci` level are recovered from the object's attributes; the only plotting-time arguments are `theme` and `se`.
* `is_doseprop_stats`: Predicate for the `doseprop_stats` class returned by `df_doseprop()`.
* S3 methods on the `doseprop_stats` class: `print()` shows a focused summary (object dimensions, regression-config attributes, the per-metric stats body); `summary()` shares the header but condenses the body to one line per metric using the `PowerCI` and `Interpretation` columns; `as.data.frame()` strips the class while preserving the metadata attributes for downstream tooling.
* `is_pmx_element`: Predicate for the shared `pmx_element` class returned by every `pmx_*()` element constructor.
* Per-type element predicates: `is_pmx_point`, `is_pmx_line`, `is_pmx_ribbon`, `is_pmx_errorbar`, `is_pmx_trend`, `is_pmx_style`, `is_pmx_color`.
* `is_pmx_theme`: Predicate for the shared `pmx_theme` class returned by every `plot_*_theme()` factory.
* Per-type theme predicates: `is_plot_dvtime_theme`, `is_plot_gof_theme`, `is_plot_dvconc_theme`, `is_plot_doseprop_theme`, `is_plot_vpc_theme`.
...
* `plot_doseprop()` now accepts the `doseprop_stats` object returned by `df_doseprop()` directly (in addition to raw observation data). Pass a precomputed `df_doseprop()` result to skip the regression refit and re-plot the same data with different `theme` / `se` settings without paying the fitting cost again. Pipeline arguments (`metrics`, `metric_var`, `exp_var`, `dose_var`, `method`, `ci`, `sigdigits`) are silently ignored on this path because the regression does not run again; `theme` and `se` are honored on both paths. Mirrors the dual-mode pattern used by `plot_vpc_cont()`.
* `df_doseprop()` carries the plotting context as attributes so the precomputed object is self-contained: `obs` (filtered observation rows), `metric_var`, `exp_var`, `dose_var`, `ci`, and `method`. `plot_build_doseprop()` reads these to render without needing the original data.
...
* Internal `validate_doseprop_stats()` helper, called at the top of `plot_build_doseprop()`, asserts that the input carries the `doseprop_stats` class and the required stats columns and attributes (`obs`, `metric_var`, `exp_var`, `dose_var`, `ci`).
* Unified panel theme application across all three plot families via internal `apply_panel_theme(plot, keep_major_x, white_panel)` helper. Each family applies its variant via parameters: continuous (`init_plot`) uses defaults; regression (`plot_build_doseprop`) passes `keep_major_x = TRUE` for log-log readability; VPC (`plot_build_vpc`) passes `white_panel = TRUE`. Replaces three separate inline `theme()` blocks with a single helper. No visual change.
