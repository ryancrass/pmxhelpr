...
plot_vpc_obs_actual <- function(sim, pcvpc = FALSE, strat_vars=NULL,
  ...
  bin_count <- sim %>% 
    dplyr::filter(MDV == 0) %>% 
    dplyr::filter(!!sym(irep_name) == 1) %>% 
    dplyr::group_by(NTIME, all_of(strat_vars)) %>% 
    dplyr::summarize(n_in_bin = n())
  ...
  if(pcvpc == FALSE){
    plot <- plot+
      ggplot2::geom_point(aes(y = OBSDV, x = TIME), data = obs, inherit.aes = FALSE,
                 shape = 1, alpha = 0.5, size = 1)
  } else {
    plot <- plot+
      ggplot2::geom_point(aes(y = PCOBSDV, x = TIME), data = obs, inherit.aes = FALSE,
                 shape = 1, alpha = 0.5, size = 1)
  }
