...
- [Getting Started with pmxhelpr](https://ryancrass.github.io/pmxhelpr/articles/getting-started.html) — a lean tour of every core workflow in one sitting.
...
You can install the most recent tagged version of pmxhelpr from [GitHub](https://github.com/ryancrass/pmxhelpr/releases/latest) with:
...
- `style_*` — returns a `ggstylekit::style_spec` (plot-family style preset)
...
Plot aesthetics are controlled with [ggstylekit](https://github.com/A2-ai/ggstylekit). Each plot function has a corresponding `style_*()` preset that returns a `ggstylekit::style_spec()` pre-filled with the plot family defaults:

  + `style_dvtime()`, `style_dvconc()`, `style_doseprop()`, `style_gof()`, `style_vpc()` — pass one to the `style` argument of the matching plot function
  + `plot_vpc_shown()` / `plot_gof_shown()` — control which VPC / GOF layers are visible via the `shown` argument

Styling is based on *keys* for plot elements to style and *roles* in the output plot (e.g. `obs_point`, `cent_line`, `cent_errorbar`, `ref_line`, `loq_line`). The per-series *keys* (`colors`, `shapes`, `sizes`, `linetypes`, `linewidths`, `alphas`) are mapped by *role*, so only *roles* specified override the defaults with others unchanged.

A finished plot can be restyled after the fact with `restyle_plot()`, and additional variables in the dataset can be surfaced with `reveal()`.
...
### `mrgsolve` Wrappers
  ...
  + `df_mrgsim_addpred()` — adds population model predictions (PRED) to the returned `data.frame`
