...
Styling is based on *maps* for plot aesthetics, which are keyed by *roles* in the output plot (e.g. `obs_point`, `cent_line`, `cent_errorbar`, `ref_line`, `loq_line`). The per-series *maps* (`colors`, `shapes`, `sizes`, `linetypes`, `linewidths`, `alphas`) are keyed by *role*, and only *roles* specified are override the defaults.
