...
Functions in this package includ the following:
...
- Wrapper functions for `mrgsolve` functions:
  *ReturnObject*\_*mrgsim*\_*Purpose*
  ...
  - `df_mrgsim_addpred()` wraps `mrgsolve::mrgsim()` to add population
    model predictions (PRED) returning a `data.frame`.
- Longitudinal Exploritory Analysis Workflow: *ReturnObject*\_*Purpose*
  - `plot_dvtime()` returns a `ggplot` object with a continuous
    dependent variable plotted versus time (e.g., PK over time, PD over
    time)
  - `plot_dvtime_dual()` returns a`ggplot` object with two separate
    continuous variables plotted versus time (e.g., PK and PD over time)
  - `plot_dvconc()` returns a `ggplot` object with a continuous
    dependent variable plotted versus an independent continuous variable
    (e.g., PD vs PK)
  - `plot_dvtime_theme()`, `plot_dvconc_theme()` return nested lists of
    modifiable default plot attributes for the `theme` arguments of
    `plot_dvtime` and `plot_dvconc`.

““(caption_dvconc)”“(caption_dvtime)
...
- VPC Workflow functions: *ReturnObject*\_*Purpose*
  ...
  - `plot_vpc_cont()` generates a `ggplot` VPC plot object from a
    simulated input dataset with exact time bins
  - `plot_vpc_legend` returns a `ggplot` object containing a legend for
    a VPC plot generated using `plot_vpc_cont`
  - `plot_vpc_theme()` returns a nested list of modifiable default plot
...
- Model Diagostic Workflow functions
...
- Vectored Helper functions: *ReturnObject*\_*Purpose*
  - `var_addn` returns a factor vector of group labels with counts of
    unique values of an identifier variable.
...
- Theme Element Helper functions: *pmx*\_*Element*
  - Elements that can be modified for each plot. Will be shown as
    optional addional documentation when calling plot-specific theme
    functions (q.g., `pmx_cent`, `pmx_vpc_line`, etc)
- Data sets
...
plot_dvtime_dual(data = data, dv_var1 = "ODV", dv_var2 = "CFB", dvid_var = "CMT",
            cent = "mean_sdl", col_var = "Regimen", log_y1 = TRUE)
