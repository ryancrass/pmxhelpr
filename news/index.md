...
- Change `dvconc_caption` and `dvtime_caption` to internal functions
  after simplifying
- Added `cfb_base` argument to `plot_popgof` (was hardcoded as
  yintercept = 0)
...
- Update `df_addn` to offload factor ordering from the function to
  simply output. Handling can be done outside the function with
  alternative packages optimized for dealing with factor variables
  (e.g., `forcats`)
- Update `plot_popgof_theme` to include separate line theme elements for
...
- Fix bug that was not scaling error bar caps for the x-axis range when
  `cent = "median_iqer` across functions
...
- Add `plot_popgof_theme` function to set and adjust default aesthetics
  for `plot_popgof`
...
- Add option to pass the same dataset variable to `TIME` and `NTIME`
  attributes in `time_vars` for all functions
- Add option to control default aesthetics (linewidth, linetype, size,
  shape, alpha) in `plot_dvtime` and `plot_popgof`
- Fix bug in `plot_popgof` that was preventing upper error bars from
...
- Fix bug in `plot_dvtime` and `plot_popgof` where variable for “DOSE”
  was being assessed when dosenorm = FALSE
- Update `breaks_time` to accept abbreviations for time units

## pmxhelpr 0.3.2

- Add `cent = "mean_sdl_upper"` option to `plot_dvtime` and
  `plot_popgof` for plotting only the upper error bar
- Add `barwidth` argument to `plot_dvtime` and `plot_popgof` to allow
  user to change error bar cap width.
...
- Added new internal package dataset `data_sad_nca` for use with
  dose-proportinality assessment functions
...
- `VPC Plot Aeesthetics` vignette demonstrating options for modifying
...
- Additional functionality for `plot_vpc_exactbins`
  ...
  - `shown` argument added to facilitate customization of which layers
    ...
    `plot_legend`. Passed to `show` argument of
...
- `plot_legend` helper function to generate a legend for a VPC plot
  ...
  which can be combined with the VPC plot using the `patchwork` package
...
- Bug fix to allow passing argument `vpcdb = TRUE` to
  [`vpc::vpc()`](https://rdrr.io/pkg/vpc/man/vpc.html)
...
- `VPC Plots with Exact Bins` vignette demostrating use case for
  [`plot_vpc_exactbins()`](https://ryancrass.github.io/pmxhelpr/reference/plot_vpc_exactbins.md)
  and workflow leveraging `df_mrgsim_replicate`.
