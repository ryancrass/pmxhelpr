# Package index

## All functions

- [`breaks_time()`](https://ryancrass.github.io/pmxhelpr/reference/breaks_time.md)
  : Determine axis breaks automatically for time variables

- [`data_sad`](https://ryancrass.github.io/pmxhelpr/reference/data_sad.md)
  : Example NONMEM Analysis-Ready Dataset for PK Modeling of a Single
  Ascending Dose Study

- [`data_sad_nca`](https://ryancrass.github.io/pmxhelpr/reference/data_sad_nca.md)
  : Example NCA Parameter Dataset Output from PKNCA

- [`data_sad_pd`](https://ryancrass.github.io/pmxhelpr/reference/data_sad_pd.md)
  : Example NONMEM Analysis-Ready Dataset for PK/PD Modeling of a Single
  Ascending Dose Study

- [`data_sad_pkfit`](https://ryancrass.github.io/pmxhelpr/reference/data_sad_pkfit.md)
  : Example NONMEM Analysis-Ready Dataset for PK Modeling of a Single
  Ascending Dose Study

- [`df_addn()`](https://ryancrass.github.io/pmxhelpr/reference/df_addn.md)
  : Add count unique values of an identifier within each level of a
  variable in a data.frame

- [`df_addpred()`](https://ryancrass.github.io/pmxhelpr/reference/df_addpred.md)
  :

  Add population predictions (`PRED`) to a data.frame

- [`df_doseprop()`](https://ryancrass.github.io/pmxhelpr/reference/df_doseprop.md)
  : Compute and tabulate estimates for log-log regression

- [`df_loglog()`](https://ryancrass.github.io/pmxhelpr/reference/df_loglog.md)
  : Compute estimate table for log-log regression

- [`df_mrgsim_replicate()`](https://ryancrass.github.io/pmxhelpr/reference/df_mrgsim_replicate.md)
  :

  Execute a visual predictive check (VPC) simulation using `mrgsolve`

- [`df_nobsbin()`](https://ryancrass.github.io/pmxhelpr/reference/df_nobsbin.md)
  : Count the non-missing observations in each exact bin

- [`df_pcdv()`](https://ryancrass.github.io/pmxhelpr/reference/df_pcdv.md)
  : Perform prediction-correction of the dependent variable

- [`dvconc_caption()`](https://ryancrass.github.io/pmxhelpr/reference/dvconc_caption.md)
  :

  Define a caption for `plot_dvconc`

- [`dvtime_caption()`](https://ryancrass.github.io/pmxhelpr/reference/dvtime_caption.md)
  :

  Define a caption for `plot_dvtime`

- [`mod_loglog()`](https://ryancrass.github.io/pmxhelpr/reference/mod_loglog.md)
  : Perform a log-log Regression

- [`model_mread_load()`](https://ryancrass.github.io/pmxhelpr/reference/model_mread_load.md)
  : Load an mrgsolve model file from the internal model library

- [`plot_doseprop()`](https://ryancrass.github.io/pmxhelpr/reference/plot_doseprop.md)
  : Plot a dose-proportionality assessment via power law (log-log)
  regression

- [`plot_dvconc()`](https://ryancrass.github.io/pmxhelpr/reference/plot_dvconc.md)
  : Plot a dependent variable versus concentration

- [`plot_dvconc_theme()`](https://ryancrass.github.io/pmxhelpr/reference/plot_dvconc_theme.md)
  : Customized Response versus drug concentration theme with pmxhelpr
  default aesthetics

- [`plot_dvtime()`](https://ryancrass.github.io/pmxhelpr/reference/plot_dvtime.md)
  : Plot a dependent variable versus time

- [`plot_dvtime_dual()`](https://ryancrass.github.io/pmxhelpr/reference/plot_dvtime_dual.md)
  : Wrapper function for plot_dvtime to plot two dependent variables
  paneled together

- [`plot_dvtime_theme()`](https://ryancrass.github.io/pmxhelpr/reference/plot_dvtime_theme.md)
  : Customized Concentration-time theme with pmxhelpr default aesthetics

- [`plot_popgof()`](https://ryancrass.github.io/pmxhelpr/reference/plot_popgof.md)
  : Plot population overlay goodness-of-fit (GOF) plots

- [`plot_popgof_theme()`](https://ryancrass.github.io/pmxhelpr/reference/plot_popgof_theme.md)
  : Customized population overlay goodness-of-fit (GOF) theme with
  pmxhelpr default aesthetics

- [`plot_vpc_exactbins()`](https://ryancrass.github.io/pmxhelpr/reference/plot_vpc_exactbins.md)
  : Plot a visual predictive check (VPC) with exact time bins

- [`plot_vpclegend()`](https://ryancrass.github.io/pmxhelpr/reference/plot_vpclegend.md)
  : Plot a legend for a visual predictive check (VPC)

- [`pmxhelpr_vpc_theme()`](https://ryancrass.github.io/pmxhelpr/reference/pmxhelpr_vpc_theme.md)
  : Customized VPC theme with pmxhelpr default aesthetics
