...
The plot can be simplified to show mean +/- standard deviation by specifying `cent = "mean_sdl"`. The observed points just add noise when combined with error bars and can be hidden by setting `alphas = c(obs_point = 0)` in the style.
...
The default aesthetics for Response-Time plots are controlled via `style_dvtime()`. See the [Plot Styling and Aesthetics](plot-styling.html) vignette for details on the styling system, the `style_spec()` fields, and examples of customizing Response-Time aesthetics.

```{r style}
style_dvtime()
```

Say we want to update the errorbar cap width to be more visible in our prior geometric mean +/- geometric SD plot with the interaction of dose and food passed to the color aesthetic. Error bar cap width is a builder argument (`errorbar_width`), while the other aesthetics come from the style.
...
```{r plot-dvtime-blq-dn-new-style, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvtime(data = plot_data_pk, dv_var = ODV, col_var = `Dose and Food`,
            cent = "mean_sdl",log_y = TRUE,
            style = dvtime_new_style, errorbar_width = 10) +
  scale_x_continuous(breaks = seq(0, 168, 24)) + 
  labs(y = "Concentration (ng/mL)", x = "Time Since First Dose (hours)") +
  facet_wrap(~PART)
```
...
The default aesthetics for Response-Concentration plots are controlled via `style_dvconc()`. See the [Plot Styling and Aesthetics](plot-styling.html) vignette for details on the styling system, the `style_spec()` fields, and examples of customizing Response-Concentration aesthetics.

```{r style-dvconc}
style_dvconc()
```

Say we want to update the color of the trend line and its standard-error ribbon. The line color is the `loess` entry of `colors`; the SE-ribbon fill is the base `line_fill`.
...
```{r plot-dvconc-new-style, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvconc(data = plot_data_pd, dv_var = CFB, idv_var = CONC,ref = 0,
            loess = TRUE, linear = FALSE, se_loess = TRUE, se_linear = FALSE,
