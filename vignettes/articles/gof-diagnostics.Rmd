...
vignette: >
  %\VignetteIndexEntry{Standard Overlay Goodness-of-Fit Diagnostics}
...
`plot_gof()` aesthetic control arguments are not reviewed in detail in this vignette. See the [Plot Styling and Aesthetics](plot-styling.html) vignette for details on customizing plot appearance.

The style preset for `plot_gof()` is `style_gof()`.

```{r style}
style_gof()
```

Say we want to add points at the binned values for the central tendency lines. The points are suppressed by default (`alpha = 0`). We can accomplish this by defining a new style and passing it to the `style` argument of `plot_gof()`.
...
```{r plot-popgof-new-style, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
