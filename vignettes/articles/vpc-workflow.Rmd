...
Build the pcVPC on `sim100` using `strat_var = PART` — the same stratification used for `cens_vpc` above. The bottom panel will carry the shared x-axis label, so the top panel sets `x = NULL`.

```{r plot-vpc-pc-strata, fig.height=4, fig.width=8, message=FALSE, warning=FALSE}
pcvpc <- plot_vpc_cont(
  data = sim100,
  pcvpc = TRUE,
  strat_var = PART
...
Stack the two diagnostics vertically with `patchwork` — pcVPC on top, cens VPC below. Both panels are built from the same `sim100` data and the same `PART` stratification, so the time bins align across the figure.

```{r plot-vpc-pc-cens-stack, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
pcvpc / cens_vpc + plot_layout(heights = c(2, 1))
```
...
pcvpc / vpc_legend / cens_vpc / cens_legend + plot_layout(heights = c(2, 0.5, 2, 0.5))
```
