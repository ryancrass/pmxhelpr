...
For downstream code or custom workflows that produce a `vpc_stats`-shaped object outside `df_vpcstats()` (e.g. external preprocessing, snapshot fixtures), `plot_build_vpc()` is exported as the public renderer. It is the same engine that `plot_vpc_cont()` uses internally and accepts the same plot-only arguments (`min_bin_count`, `show_rep`, `shown`, `style`, `pcvpc`). `strat_var` and `loq` inherit from the container's `$config` slot when not passed explicitly.
...
The default aesthetics for VPC plots are controlled via `style_vpc()`. See the [Plot Styling and Aesthetics](plot-styling.html) vignette for details on the styling system, the `style_spec()` fields, and examples of customizing VPC aesthetics.

```{r vpc-style}
style_vpc()
```

We can define an alternative style using a blue / grey color schema. VPC roles are keyed in the per-series maps: line/point colors in `colors` and ribbon fills in `fill`.
...
Regenerating our BLQ quantile censored plot stratified by part with the new style yields the following plot

```{r plot-vpc-blq-obs-cens-newstyle, fig.height=4, fig.width=8, message=FALSE, warning=FALSE}
vpc2 <- plot_vpc_cont(
  data = sim100,
  strat_var = PART,
  loq = 1,
...
`plot_vpc_cens()` reuses `plot_vpc_shown()` and `style_vpc()` — only the keys `obs_point`, `obs_median_line`, `sim_median_line`, and `sim_median_ci` are read with other keys ignored.
...
The legend can then be combined with the ggplot object returned from `plot_vpc_cont()` into a single plot object with the `patchwork` package. We pair the default-style legend with `vpc` (the LOQ-censored, PART-stratified plot from the [BLQ handling](#blq-handling) section, which also uses the default style).
...
The legend can also be updated with the same VPC style. This is easiest to do by reusing the `vpc_new_style` object we created earlier with `style_vpc()`. We pass the same style to both `plot_vpc_cont()` and `plot_vpc_legend()`, again passing `lloq = 1` to keep the legend in sync with the LLOQ ref line on `vpc2`.
...
The legend can then be combined with the ggplot object returned from `plot_vpc_cont()` into a single plot object with the `patchwork` package. We pair `vpc_new_legend` (which uses `vpc_new_style`) with `vpc2` (the same VPC plot rendered with `vpc_new_style` earlier).
...
By default `sim_median_line` is hidden (`plot_vpc_shown()` sets it to `FALSE`) and `sim_median_ci` inherits the same red fill as `obs_median_line` from `style_vpc()`. This is fine when only the ribbon is shown, but enabling the simulated median line on top of the default style places a black dashed line over a red ribbon next to a red observed line. With these aesthetics, the red observed line could easily be mistaken for a simulation element given color homology with the simulated interval.
...
`plot_vpc_cens()` reads four `shown` keys (`obs_point`, `obs_median_line`, `sim_median_line`, `sim_median_ci`) and the corresponding style roles. Build matching `cens_shown` and `cens_style` objects so the change is explicit and reusable:

```{r vpc-cens-shown-style}
...
```{r plot-vpc-cens-styled, fig.height=4, fig.width=8, message=FALSE, warning=FALSE}
...
`plot_vpc_legend()` accepts the same `shown` and `style` objects, so passing them alongside `type = "cens"` keeps the legend in sync with whatever combination of visibility toggles and aesthetic overrides was used for the panel. The `Sim Prop BLQ` entry now appears (because `sim_median_line` is on) and both simulated entries adopt the unified blue.

```{r vpc-cens-legend-styled, fig.height=4, fig.width=8, message=TRUE, warning=FALSE}
