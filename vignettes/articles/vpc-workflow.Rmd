...
# VPC Workflow
 ...
 `pmxhelpr` includes two functions for calculating summary statistics across simulated replicates of an input dataset and plotting the ouput for model evaluation: `df_vpcstats` and `plot_vpc_cont()`.
...
### Restricting Carried-Forward Variables

By default, `df_mrgsim_replicate` auto-carries every numeric and character column of `data` into the output, in addition to the always-carried set (`CMT`, `EVID`, `MDV`, `TIME`, `NTIME`, `ID`, plus the model output / DV columns). This means downstream functions like `plot_vpc_cont()` find columns such as `LLOQ`, `BLQ`, and stratification variables without further configuration.

If you want a leaner output containing only specific columns, pass `num_vars` / `char_vars` as character vectors:

  - `num_vars` --- numeric variable names passed to the `carry_out` argument of `mrgsim_df()`.
  - `char_vars` --- character variable names passed to the `recover` argument of `mrgsim_df()`.

When either is non-`NULL`, that side becomes the explicit list (only those columns are carried). Passing `NULL` (the default) restores auto-carry for that side.
...
`pmxhelpr` contains a function to pre-process and derive summary statistics from the observed and simulated data for plotting in downstream VPC and pcVPC plots to evalaute the model fit for longitudinal, continuous repeated measures data: `df_vpcstats()` 
...
VPC summary statistics are calculated for the input dataset `data` at the study replicate replicate-level at the median and quantiles specified by `pi`, and subserquently summarized across replicates at the median and upper and lower quantiles specified by `ci` to identify the non-parametric confidence interval. 
...
The handling of BLQ imputation for derivation of quantiles within and across replicates is controlled by the `mode` argument. Options include:
  ...
  + `"drop"` drop BLQ data and calculate quantiles on the continuous portion of the data
...
For standard VPCs without prediction-correction, mboth the observed and simulated data are handled with `"rank"`; however, no BLQ handling is applied to the simulated data and rankings are based on raw simulated values without BLQ censoring to illustrate the underlying "true" profile based on the model.
...
glimpse(vpcstats_ntime_dose_food_drop$stats)
...
glimpse(dplyr::select(vpcstats_ntime_part$stats, BIN_MID, PART, dplyr::starts_with("pc_")))
```
...
plot_vpc_cont(out, pcvpc = TRUE) +
  labs(x = "Time (hours)", y = "Prediction-corrected concentration") +
  scale_x_continuous(breaks = seq(0, 168, 24))
```
...
out <- df_vpcstats(data = simout, strat_var = DoseFood)
plot_build_vpc(out, pcvpc = FALSE) +
  labs(x = "Time (hours)", y = "Concentration (ng/mL)") +
  scale_x_continuous(breaks = seq(0, 168, 24)) +
  scale_y_log10(guide = "axis_logticks")
```
...
Simulated and observed intervals in the plot below are *INCORRECT* and reflect the full dataset, not the `DoseFood` stratification. Only the observed points are correctly stratified, as they are added to the final plot layer.
...
`plot_vpc_cont()` returns a `ggplot` object. To access the underlying summary statistics directly, use `df_vpcstats()` — it accepts the same simulation input and returns a `pmx_stats` container with `stats` (observed and simulated quantiles, confidence intervals, and bin information), `obs` (first-replicate observation rows used as the plot's scatter overlay), and `config` (the run configuration) slots.
...
vpc_new_legend <- plot_vpc_legend(theme = vpc_new_theme)
vpc_new_legend
```
...
# Pipeline Column Contracts

Reference table of column-name expectations across the three VPC pipeline stages. Default column names are shown; each user-facing function accepts column-name arguments (`time_var`, `ntime_var`, `pred_var`, `sim_dv_var`, `obs_dv_var`, `irep_name`, `strat_var`) that remap input columns to the standard names used internally. Output names are fixed.

## `df_mrgsim_replicate(data, model, replicates, ...)`

| Stage | Columns |
|---|---|
| **Required input** | `ID`, `TIME`, `NTIME`, `EVID`, `MDV`, `CMT`, plus the column named by `dv_var` (default `DV`) |
| **Always-carried output** | `ID`, `TIME`, `NTIME`, `EVID`, `MDV`, `CMT`, `PRED`, `IPRED`, `SIMDV`, `OBSDV`, replicate counter (default `SIM`) |
| **Auto-carried output** | All numeric and character columns of `data` (e.g. `LLOQ`, `DOSE`, `FOOD`, `PART`) — restricted with explicit `num_vars` / `char_vars` lists |

Output column-name standardization is the key contract: `dv_var` is renamed to `OBSDV` and the simulator's DV capture is renamed to `SIMDV`, regardless of source names.

## `df_vpcstats(data, ...)`

| Stage | Columns |
|---|---|
| **Required input** | `EVID`, `MDV`, `PRED`, `SIMDV`, `OBSDV`, `TIME`, `NTIME`, `SIM` (or per `irep_name`) |
| **Optional input** | `LLOQ` (auto-inherited as `loq`), `<strat_var>` (any column named via `strat_var`) |
| **Output: `$stats`** | Identifiers: `BIN_MID`, `<strat_var>` |
|  | Counts: `obs_n`, `obs_n_blq`, `obs_prop_blq` |
|  | Std observed quantiles: `obs_low`, `obs_med`, `obs_hi` |
|  | Std simulated CIs: `sim_low_low/med/hi`, `sim_med_low/med/hi`, `sim_hi_low/med/hi` |
|  | Std sim BLQ proportions: `sim_prop_blq_low/med/hi` (only when `loq` is known) |
|  | pcVPC observed quantiles: `pc_obs_low/med/hi` |
|  | pcVPC simulated CIs: `pc_sim_low_low/med/hi`, `pc_sim_med_low/med/hi`, `pc_sim_hi_low/med/hi` |
|  | Run config: `ci`, `pi_low`, `pi_hi` |
| **Output: `$obs`** | First-replicate observation rows with `MDV == 0`. Carries `OBSDV` (std-scale), `PC_OBSDV` (prediction-corrected), `TIME`, plus all input columns. |
| **Output: `$config`** | Named list: `n_replicates`, `loq`, `strat_var` |

## `plot_build_vpc(compute_out, ...)`

| Stage | Columns |
|---|---|
| **Required input** | A `vpc_stats` container with the `$stats`, `$obs`, and `$config` columns above. Validated at entry by the internal `validate_vpc_stats()`. |
| **Honored args on the precomputed path** | `min_bin_count`, `show_rep`, `shown`, `theme`, `pcvpc` |

Pipeline arguments (`strat_var`, `loq`, `mode`, `pi`, `ci`, column-name args) are **not** accepted at this stage — those decisions live in `df_vpcstats()`. Passing any of them to `plot_vpc_cont()` on the precomputed-stats path aborts with a message pointing back at `df_vpcstats()`.
