...
The `df_vpcstats` function can be called within `plot_vpc_cont()`, for a one function call calculation and visualization of VPC statistics. `df_vpcstats` returns a list of two data.frames, `stats` and `obs`, which contained the summary statistics of the simulated and observed data and the observed data points for overlay, respectively. The returned output can be passed directly into `plot_vpc_cont()` or the full pipeline can be run within the plotting function.
...
`df_vpcstats()` returns a list with two data.frames: 
...
The `df_vpcstats()` return is class-tagged `"vpc_stats"`; `print(out)` shows a focused summary of dimensions, run-config attributes (`n_replicates`, `loq`, `strat_var`), and column groups rather than dumping all columns. Use `out$stats` and `out$obs` (or `as.data.frame(out)` for the stats data.frame) to access the underlying frames.
...
When `data` is a precomputed `df_vpcstats()` result, the pipeline arguments (`strat_var`, `loq`, `mode`, `pi`, `ci`, column-name args) are ignored — those decisions were already made when `df_vpcstats()` ran. Plot-only arguments (`min_bin_count`, `show_rep`, `shown`, `theme`, `pcvpc`) still apply.
...
`plot_vpc_cont()` returns a `ggplot` object. To access the underlying summary statistics directly, use `df_vpcstats()` — it accepts the same simulation input and returns a list with two data.frames: `stats` (observed and simulated quantiles, confidence intervals, and bin information) and `obs` (first-replicate observation rows used as the plot's scatter overlay).
