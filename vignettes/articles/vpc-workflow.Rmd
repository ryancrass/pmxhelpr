...
Two arguments to `mrgsim_df()` are reserved and cannot be passed through `...`: `carry_out` and `recover`. These are managed internally to populate the output columns selected by `num_vars` and `char_vars`. Attempting to pass either through `...` aborts with an informative error pointing to the correct argument.

```{r df_mrgsim_replicate-carry-out-guard, error=TRUE}
df_mrgsim_replicate(data = data_sad_pk,
                    model = model,
                    replicates = 100,
                    dv_var = ODV,
                    carry_out = "DOSE")
