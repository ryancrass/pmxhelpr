...
The unique vector of values for `data$loq` is used automatically if the variable `"LLOQ"` is present in the input dataest. This allows for the scenario in which there are multiple unique LLOQ values within a single pooled analysis dataset.
...
vpc_multi <- plot_vpc_cont(out_multi) +
  labs(x = "Time (hours)", y = "Concentration (ng/mL)") +
  scale_x_continuous(breaks = seq(0, 168, 24)) +
  scale_y_log10(guide = "axis_logticks")
vpc_multi
...
To obtain a legend for a plot using default aesthetics, run `plot_vpc_legend()`. Pass `lloq` to include an LLOQ entry that mirrors the reference line drawn by `plot_vpc_cont()` when `loq` was specified — here we pass `lloq = 1` to match the `loq = 1` used to build `vpc` above.
