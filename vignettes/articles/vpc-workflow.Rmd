...
By default, `df_mrgsim_replicate` auto-carries every numeric and character column of `data` into the output. This means downstream functions like `plot_vpc_cont()` find columns such as `LLOQ`, `BLQ`, and stratification variables without further configuration.

If you want a leaner output containing only specific columns, pass character vectors of numeric and character variable names to `num_vars` and `char_vars`, respectively. 
...
plot_vpc_cont(out, pcvpc = TRUE) +
  labs(x = "Time (hours)", y = "Prediction-corrected concentration (ng/mL)") +
  scale_x_continuous(breaks = seq(0, 168, 24)) +
  scale_y_log10(guide = "axis_logticks")
```
