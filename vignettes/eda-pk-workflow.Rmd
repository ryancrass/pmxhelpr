...
The actual time since first dose administration is assigned to the NONMEM reserved variable name for time (`TIME`), as this represents the most accurate description of the time order of the repeated-measures data. The nominal time variables are *exact binning variables*, which are useful for grouping data for exploratory data analysis or model evaluation....but more on that later.
...
Let's define some new variables and count unique subjects in each for use in plotting. Useful study design variables for stratifying plots include: dose (DOSE), food status (FOOD), and their interaction since subjects receive 100 mg under both fed and fasted conditions.
...
Additionally, let's filter down only to PK observations for this exploratory analysis, which is focused on assessing the informational content of the data and trends for population PK analysis.
...
Let's try passing in our PK dataset and see what happens!
...
Hmm...that didn't work. `plot_dvtime` has 3 arguments that specify the dependent variable to be mapped to the y-axis `dv_var`, the independent variable to be mapped to the x-axis (`time_var`), and a binned version of the x-axis variable for calculation of central tendency statistics (`ntime_var`). The defaults are as follows:

  - `dv_var` = DV
  - `time_var` = TIME
  - `ntime_var` = NTIME
  
`plot_dvtime` supports non-standard evaluation and these arguments may be passed as bare names or strings.

The `dv_var` specifies the dependent variable in the dataset with default of `DV`. This argument uses non-standard evaluation and can be passed as a bare column names or as a string. 

Since `data_sad` has the dependent variable mapped to the name `ODV`, this name must be passed to the function.

```{r plot-dvtime-pass, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvtime(plot_data_pk, dv_var = ODV)
```

Now the function returns a plot! Conveniently, the time variables in `data_sad` had names aligned with the default argument; however, this may not always be the case. 

## Time Variables

The names of the actual and nominal time variables can be specified with the `time_var` and `ntime_var` arguments, which accept bare names or strings. The same variable name can be passed to each argument to handle the special case where only nominal times are known.
...
The plot can be simplified to show mean +/- standard deviation by specifying `cent = "mean_sdl"`. The observed points just add noise when combined with error bars and can be removed by specifying `obs_dv = FALSE`.
...
plot_dvtime(data = plot_data_pk, dv_var = ODV, col_var = `Dose and Food`, 
            cent = "mean_sdl_upper",obs_dv = FALSE) +
  scale_x_continuous(breaks = seq(0, 168, 24)) + 
  labs(y = "Concentration (ng/mL)", x = "Time Since First Dose (hours)") +
  facet_wrap(~PART)
```
...
plot_dvtime(data = plot_data_pk, dv_var = ODV, col_var = `Dose and Food`, 
            cent = "median_iqr",log_y = TRUE, obs_dv = FALSE) +
  scale_x_continuous(breaks = seq(0, 168, 24)) + 
  labs(y = "Concentration (ng/mL)", x = "Time Since First Dose (hours)") +
  facet_wrap(~PART)
```
...
Data points may be connected longitudinally within an individual by specifying as grouping variable `grp_var` (default is `ID`) which will be connected when `grp_dv = TRUE`. The grouping variable inherits the variable mapped to the colore aesthetic with `col_var`. The central tendency may be removed by specifying `cent ="none"`or the median may be plotted by specifying `cent="median"`
...
plot_dvtime(plot_data_pk, dv_var = ODV, col_var = `Dose and Food`, cent = "mean",
            ...
            dosenorm = TRUE) +
  scale_x_continuous(breaks = seq(0, 168, 24)) + 
  labs(y = "Dos-normalized Conc. (ng/mL/mg)", x = "Time Since First Dose (hours)") +
  facet_wrap(~PART)
```
...
The default attributes of data points and lines are controlled by the `theme` argument. The defaults can be viewed by running `plot_dvtime_theme()` with no arguments
...
plot_dvtime(plot_data_pk, dv_var = ODV, col_var = `Dose and Food`, 
            cent = "none",log_y = TRUE, grp_dv = TRUE,
            loq_method = 2, loq = 1) +
