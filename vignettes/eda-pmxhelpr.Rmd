...
The wrapper function `plot_dvtime_dual` is provided to support simultaneous plotting of PK and PD profiles for visualization of trends in time.

```{r plot-dvtime-dual-col, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvtime_dual(plot_data, dv_var = "ODV", dvid_var = "CMT", dvid_values = c(DV1 = 2, DV2 = 3),
                 col_var = "Dose") 
```
...
Similarly, specification of a reference line at y=0 for change from baseline is only supported for the bottom panel (DV2).

```{r plot-dvtime-dual-col-y2, fig.height=6, fig.width=6, message=FALSE, warning=FALSE}
plot_data_cfb <- plot_data %>% 
  mutate(CFB = ifelse(CMT == 2, ODV, CFB))

plot_dvtime_dual(plot_data_cfb, dv_var = "CFB", dvid_var = "CMT", dvid_values = c(DV1 = 2, DV2 = 3),
