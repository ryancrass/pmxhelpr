...
The helper function `df_addn` takes an input dataset `data` and returns a dataset with the variable specified in `grp_var` transformed to a factor including the count of unique values of the identifier variable specified in `id_var` (default `ID`). Both arguments use non-standard evaluation and can be passed as bare column names or strings, as shown below.
...
The color aesthetic can be mapped to a dataset variable using the the `col_var` argument. This argument uses non-standard evaluation and can be passed as a bare column name or as a string. Let's use a variable we defined earlier using `df_addn` to list unique study conditions.
