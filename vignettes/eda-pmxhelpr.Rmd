...
tab <- tab_doseprop(nca_results_obj, metrics = c("aucinf.obs", "cmax"))


ggplot(data = auc, aes(x = DOSE, y = PPORRES)) +
    geom_point() +
    geom_smooth(method = "lm", formula = y~x) +
    labs(x = "Dose", y = "Exposure", 
         title = paste0("Power: ", signif(fit_auc_df$Power, digits=3), 
                        " (",fit_auc_df$CI," ", signif(fit_auc_df$LCL, digits=3), " - ", 
                        signif(fit_auc_df$UCL, digits=3), ")")) +
    scale_x_log10() +
    scale_y_log10() +
    theme_bw()
```
