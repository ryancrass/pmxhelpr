...
tmp_plot_data <- data_sad %>% 
  filter(EVID == 0) %>% 
  mutate(`Food Status` = ifelse(FOOD == 0, "Fasted", "Fed"), 
         `Dose and Food` = paste0(DOSE, " mg ", `Food Status`),
         `Dose` = DOSE)

levels_dosefood <- unique(plot_data$`Dose and Food`)
levels_dosefood

plot_data <- tmp_plot_data %>% 
  df_addn(grp_var = "Dose", sep = "mg") %>% 
  df_addn(grp_var = "Dose and Food", levels = levels_dosefood)
...
plot_dvtime(plot_data, dv_var = "ODV")
```
...
plot_dvtime(data = plot_data, dv_var = "ODV", show_caption = FALSE) 
```
...
```{r plot-dvtime-nocap, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvtime(data = plot_data, dv_var = "ODV", log_y) 
```
...
plot_dvtime(plot_data, dv_var = "ODV", time_vars = c(TIME = "NTIME"))
```
...
plot_dvtime(data = plot_data, dv_var = "ODV") 
```
...
plot_dvtime(data = plot_data_days, dv_var = "ODV", timeu = "days") 
```
...
The color aesthetic csn be mapped to a dataset variable using the the `col_var` argument. Let's use a variable we defined earlier using `df_addn` to list unique study conditions.

```{r plot-dvtime-col, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvtime(data = plot_data, dv_var = "ODV", log_y = TRUE, col_var = "Dose", )
```

The argument `cent` specifies the method of calculating the central tendency (+/- variability) within levels of the variable passed to the `col_var`. The default is `cent = "mean"`; however, note that the calculation performed are *after* any transformations to the data and this option will return the geometric mean when `log_y=TRUE`. If the `log_y` argument is used to generate semi-log plots along with `show_captions = TRUE`, then the caption will  delineate where arithmetic and gemoetric means are being returned. 

```{r plot-dvtime-part-log, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvtime(data = plot_data, dv_var = "ODV", col_var = "Dose", cent = "mean", 
...
plot_dvtime(data = plot_data, dv_var = "ODV", col_var = "Dose", cent = "mean_sdl", 
            ylab = "Concentration (ng/mL)", log_y = TRUE,
            obs_dv = FALSE) +
  facet_wrap(~PART)
```
...
plot_dvtime(data = plot_data, dv_var = "ODV", col_var = "Dose", cent = "mean_sdl_upper", 
...
plot_dvtime(data = plot_data, dv_var = "ODV", col_var = "Dose", cent = "median_iqr", 
            ylab = "Concentration (ng/mL)", log_y = TRUE,
            obs_dv = FALSE) +
  facet_wrap(~PART)
```
...
plot_dvtime(data = plot_data, dv_var = "ODV", col_var = "Dose", cent = "median", 
            ylab = "Concentration (ng/mL)", log_y = TRUE, 
            grp_dv = TRUE) +
  facet_wrap(~PART)
```
...
plot_dvtime(plot_data, dv_var = "ODV", col_var = "Dose", cent = "mean",
            ylab = "Concentration (ng/mL)", log_y = TRUE,
            loq_method = 2) +
  facet_wrap(~PART)
```
...
plot_dvtime(select(plot_data, -DOSE), 
            dv_var = "ODV", col_var = "Dose", cent = "mean",
            ylab = "Dose-normalized Conc. (ng/mL per mg Drug)", log_y = TRUE,
            dosenorm = TRUE) +
  facet_wrap(~PART)
```
...
ggplot(plot_data, aes(x = TIME, y = ODV, col = interaction(`Dose`, `Food Status`))) +
  geom_point()+
...
Let's visualize the data. First, we will filter to response observation records only (CMT=3) and derive some factor variables, which can be passed to the color aesthetic in our plots.

```{r data-plotdata-pd}
plot_data <- data_sad_pd %>% 
  filter(EVID == 0) %>% 
  mutate(`Food Status` = factor(FOOD, levels = c(0, 1), labels = c("Fasted", "Fed")), 
         Dose = DOSE)

plot_data <- df_addn(plot_data, grp_var = "Dose", id_var = "ID", sep = "mg")
plot_data_pd <- filter(plot_data, CMT == 3)
```

Let's first visualize these data versus time using `plot_dvtime`.

```{r plot-dvtime-pd, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvtime(data = plot_data_pd, dv_var = "CFB", col_var = "Dose", ylab = "Response") 
```


Now let's visualize the response-concentration data. `pmxhelpr` includes a function for common visualizations of observed response versus concentration data in exploratory data analysis: `plot_dvconc`

In our visualizations, we will leverage the following dataset variables: 

  + `ODV`: the original dependent variable (biomarker respponse) normalized to baseline
  + `ICONC` : individual PK model-predicted concentration 
  
`plot_dvconc` requires a dependent variable, specified as string via the `dv_var` argument, and an independent variabl, specified as a string via the `idv_var` argument. The default values are `"DV"` and `"CONC"`.

```{r plot-dvconc, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvconc(data = plot_data_pd, dv_var = "CFB", idv_var = "ICONC") 
```

## Specifying the Color Aesthetic

`plot_dvconc` includes two arguments for controlling the color aesthetic, `col_var` and `col_trend`. If a variable is passed as a string to the `col_var` argument, the data points are colored based on this variable; however, by default `col_trend = FALSE` and the trend line is fit without the color aesthetic.
...
This behavior can be modified by setting `col_trend = TRUE`.
...
There are two trend line types supported by `plot_dvconc`, LOESS smoothing fit and linear regression controlled by logical arguments `loess` and `linear`. The default is `loess = TRUE` and `linear = FALSE`

```{r plot-dvconc-col-trend-loess-linear, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
plot_dvconc(data = plot_data_pd, dv_var = "CFB", idv_var = "ICONC", 
            col_var = "Dose", col_trend = FALSE, loess = TRUE, linear = TRUE) 
```

The 95%CIs of the trendlines are suppressed by default. They can be added to the plot using the logical arguments `se_loess` and `se_linear`.
...
Additional arguments can be passed to `geom_smooth(method = "loess")`, such as increasing the span of the smoother.
...
If the color aesthetic is mapped to the trendlines with `col_trend = TRUE`, it will also mapp to the ribbons defining the 95% CIs. 
