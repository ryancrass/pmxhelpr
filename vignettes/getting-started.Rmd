...
See the [Exploratory Analyses of PK and PK/PD Data](eda-pk-pkpd-workflow.html) article for central tendency controls, BLQ imputation options, dose-normalization, and other visual controls.
...
From a validated model file, the VPC pipeline includes:
    + replication of the input dataset via simulation with `df_mrgsim_replicate()`
...
pmxhelpr styles its plots with [ggstylekit](https://github.com/A2-ai/ggstylekit). Every `plot_*()` function has a paired `style_*()` preset that returns a `ggstylekit::style_spec()` pre-filled with the family's defaults. Styling is keyed by the plot's *roles* (e.g. `obs_point`, `cent_line`, `cent_errorbar`, `ref_line`, `loq_line`): the per-series maps (`colors`, `shapes`, `sizes`, `linetypes`, `linewidths`, `alphas`) are keyed by role, and partial overrides merge onto the defaults (so setting one role leaves the others untouched).
...
A finished plot can also be restyled after the fact with `ggstylekit::restyle_plot()`, or a hidden covariate surfaced with `ggstylekit::reveal()`.
