---
title: "Standard PopPK Overlay Goodness-of-Fit Diagnostics Workflow"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{gof-diagnostics-pk}
...
For this vignette, we will generate model diagnostics for a PK model (`pkmodel`) fit to the `data_sad` dataset using the `data_sad_pkfit` dataset internal to `pmxhelpr`. We can take a quick look at the dataset using `glimpse()` from the dplyr package. Dataset definitions can also be viewed by calling `?data_sad_pkfit`, as one would to view the documentation for a package function.
...
Column mapping is specified through individual arguments that accept bare names or strings:
...
There is only one new argument to `plot_popgof` that is not present in `plot_dvtime`. This argument is `output_colors`, a named vector of pre-defined plot element and color pairs:
...
plot_popgof(data = plot_data, dv_var = "ODV", obs_dv = FALSE, cent = "median_iqr") +
  facet_wrap(~DoseGroup, scales = "free") +
  labs(y = "Concentration (ng/mL)")
```
...
plot_popgof(data = plot_data, dv_var = "ODV", cent = "mean", grp_dv = TRUE,
            log_y = TRUE, loq_method = 2) +
  facet_wrap(~DoseGroup, scales = "free") +
  labs(y = "Concentration (ng/mL)")
```
...
for(i in 1:n_ids){
  ...
  labs(title = paste0("ID = ", ids[i]), y = "Concentration (ng/mL)") +
