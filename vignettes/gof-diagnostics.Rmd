...
The default aesthetics for each output variable (OBS, DV, PRED, IPRED) are controlled via `plot_gof_theme()`. See the [Plot Themes and Aesthetics](plot-aesthetics.html) vignette for details on the theme system, element constructors, and examples of customizing output colors.

The observed data points can be removed with only the summary metrics plotted using `obs_dv = FALSE`. Notice how different this plot appears when only visualizing central tendency!
