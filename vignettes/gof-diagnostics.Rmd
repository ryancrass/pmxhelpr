...
The `shown` argument controls which output variables are displayed. It accepts a named logical vector with names: `"OBS"`, `"DV"`, `"IPRED"`, and `"PRED"`. All default to `TRUE`. 
...
plot_gof(data = plot_data, dv_var = "ODV", shown = c(IPRED = FALSE)) +
  facet_wrap(~`Dose and Food`, scales = "free") +
  labs(y = "Concentration (ng/mL)", x = "Time Since First Dose (hours)")
```

The observed data points can also be removed with `obs_dv = FALSE`. Notice how different this plot appears when only visualizing central tendency!
...
The default aesthetics for VPC plots are controlled via `plot_gof_theme()`. See the [Plot Themes and Aesthetics](plot-aesthetics.html) vignette for details on the theme system, element constructors, and examples of customizing VPC aesthetics.
...
gof_new_theme <- plot_gof_theme(
  ...
  pred_point = pmx_point(shape = 16, alpha = 1),
  ipred_point = pmx_point(shape = 16, alpha = 1),
  dv_point = pmx_point(shape = 16, alpha = 1)
)
```
...
The previous section provides an overview of how to generate population overlay GOF plots using `plot_gof()`; however, we can also use `plot_gof()` to generate subject-level visualizations with a little pre-processing of the input dataset. The `shown` argument can be used to suppress the population prediction (PRED) in these plots evaluating fit at the individual level.
