...
## Overview
 ...
 `plot_gof` inherits its data handling and aesthetic control arguments from `plot_dvtime`. The key additions are `pred_var`, `ipred_var`, `shown`, and `theme` (via `plot_gof_theme()`). See the [Plot Themes and Aesthetics](plot-aesthetics.html) vignette for details on customizing plot appearance.
...
The `shown` argument controls which output variables are displayed. It accepts a named logical vector with names `"OBS"`, `"DV"`, `"IPRED"`, and `"PRED"`. All default to `TRUE`. For example, `shown = c(IPRED = FALSE)` hides the IPRED layer.

The default colors and aesthetics for each variable are controlled via the `theme` argument and `plot_gof_theme()`. See the [Plot Themes and Aesthetics](plot-aesthetics.html) vignette for details on customizing output colors.
