...
This vignette will review the limitations of the `vpc()` function in the `vpc` package for datasets containing exact bins as the motivating reason for development `plot_vpc_exactbins()`. The `vpc::vpc()`function offers a lot of great functionality to generate VPC plots including combining the data processing and plotting steps into one function call, built in options for automatic binning, an easy option for prediction-correction, and functionality for processing data accounting for censoring at the lower limit of quantification (LLOQ). 

However, although `vpc()` contains many great options to automatically identify bins in the data, it is not optimized for input data containing exact bins. 
...
Luckily for us, someone has already fit a PK model to these data! Let's load the mrgsolve model file internal to the pmxhelpr package by calling `model_mread_load()` and take a look at it with `mrgsolve::see()`.
...
Glimpsing the output from `df_mrgsim_replicate()` reveals the following model outputs: 
...
We can also pass stratifying variables to the argument `strat_var` in order to facet our plots by relevant extrinsic study design covariates or intrinsic subject covariates. The stratify variables specified in `strat_var` are also passed to the `stratify` argument of `vpc()` in order to facet the resulting plots. Currently, only one variable can be passed to this argument.
