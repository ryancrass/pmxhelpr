...
This vignette will review the functionality of `plot_vpc_exactbins()` for generating VPC plots using exact time bins from datasets containing a nominal time variable.
...
`plot_vpc_exactbins()` uses the unique values of the nominal time variable directly as exact bins, ensuring that summary statistics are calculated at the actual protocol-specified sampling times.
...
`plot_vpc_exactbins()` uses the unique values of the nominal time variable (`NTIME`) directly as exact bins. This ensures that summary statistics are calculated at each protocol-specified sampling time, preserving the full resolution of the data.
...
Because `plot_vpc_exactbins()` returns a `ggplot2` object, the y-axis can be transformed to log10 scale by adding `scale_y_log10()` as a layer. This approach avoids any data transformation prior to calculating summary statistics, ensuring all quantile calculations are performed on the original scale.
