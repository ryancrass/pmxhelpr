...
This vignette will review the excellent functionality in the `vpc()` function (`vpc` package) for appropriate handling of data censored below the lower limit of quantification (LLOQ) using the `lloq` argument. `vpc_plot_exactbins()` takes advantage of this functionality through the alias `loq` argument.
