...
Now, suppose we want to change the default aesthetics of the VPC plot, without changing what is being shown. This can be accomplished by passing a named list of elements to update to the function `plot_vpc_theme()`. The named list object generated from this function can then be passed to the `theme` argument in `plot_vpc_exactbins`.

Let's say we prefer a grey-blue color scheme. We can customize it by passing a named list to `plot_vpc_theme()`:
...
vpc_pc_customtheme <- plot_vpc_exactbins(
  ...
  output_vars = c(PRED = "PRED", IPRED = "IPRED", SIMDV = "SIMDV", OBSDV = "OBSDV"),
  pcvpc = TRUE,
  vpc_theme = plot_vpc_theme(update = list(obs_color = "#000000",
