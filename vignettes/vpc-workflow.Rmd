...
# VPC Workflow with `plot_vpc_cont()`
...
`plot_vpc_cont()` uses the unique values of the nominal time variable directly as exact bins, ensuring that summary statistics are calculated at visualized at the protocol-specified sampling times.
...
This is useful for tabulating VPC diagnostics or creating custom visualizations outside the default `plot_vpc_cont()` output.
...
`plot_vpc_cont()` returns a `ggplot2` plot object, which can be modified by adding layers just like any other `ggplot2` object. For example, to add x-axis grid lines with a custom theme:
